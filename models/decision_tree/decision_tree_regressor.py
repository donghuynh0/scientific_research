...
def relative_error_criterion(y_left, y_right, feature_idx=None, threshold=None, rho_star_idx=None, rho_penalty_weight=1):
...
class CustomDecisionTreeRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, max_depth=10, min_samples_split=2,
                 criterion=relative_error_criterion,
                 rho_star_idx=None, rho_penalty_weight=1):
        self.tree_ = None
        self.max_depth = max_depth
        self.min_samples_split = min_samples_split
        self.criterion = criterion
        self.rho_star_idx = rho_star_idx
        self.rho_penalty_weight = rho_penalty_weight
        ...
        for feature in range(X.shape[1]):
            ...
            for t in thresholds:
                ...
                score = self.criterion(y[left], y[right],
                                       ...
                                       rho_penalty_weight=self.rho_penalty_weight)
                if score < best_score:
                    best_score = score
                    best_split = {
                        'feature': feature,
